...
public class ExamController {
    ...
    public ResponseEntity<Response<ExamDTO>> createExam(
        ...
        try {
            ...
            // 设置创建者ID
            if (currentUser != null) {
                // 由于Exam.creator引用的是Teacher实体，需要获取对应的教师ID
                // 如果当前用户是管理员，可以选择指定教师创建考试；如果是教师，则使用自己的ID
                if (examDTO.getCreatorId() == null || examDTO.getCreatorId() <= 0) {
                    // 如果前端没有指定创建者，或创建者ID无效，使用当前教师的ID
                    if (currentUser instanceof Teacher) {
                        examDTO.setCreatorId(currentUser.getId());
                    } else {
                        // 如果当前用户不是教师，查找对应的教师记录
                        ...
                        if (teacherOpt.isPresent()) {
                            ...
                            return ResponseEntity.badRequest().body(Response.error("当前用户不是教师，无法创建考试"));
                        }
                    }
                }
