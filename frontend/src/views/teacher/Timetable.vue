      ...
      >
        <div v-if="selectedCourse" class="course-detail">
          <el-descriptions :column="1" border>
            ...
            <el-descriptions-item label="授课班级">{{ selectedCourse.className }}</el-descriptions-item>
            ...
            <el-descriptions-item label="学生人数">{{ selectedCourse.studentsCount }}</el-descriptions-item>
            <el-descriptions-item label="课程进度">{{ selectedCourse.progress }}%</el-descriptions-item>
    ...
    async loadCourses() {
      try {
        ...
        if (response.data.success) {
          ...
          // 获取每个课程的详细安排信息
          const courseSchedulesPromises = courses.map(course => 
            this.$axios.get(`/course-schedules/by-course/${course.id}`, {
              headers: {
                'Authorization': `Bearer ${localStorage.getItem('token')}`
              }
            })
          ...
          const courseSchedulesResponses = await Promise.all(courseSchedulesPromises);
          
          // 整合课程和课程安排数据
          this.courses = [];
          courseSchedulesResponses.forEach((res, index) => {
            if (res.data.success) {
              const courseSchedules = res.data.data || [];
              ...
              courseSchedules.forEach(schedule => {
                this.courses.push({
                  ...
                  studentsCount: course.studentCount || 0,
                  courseType: course.courseType,
                  description: course.description,
                  schedule: `${this.getDayLabel(schedule.day)} ${this.getSectionTime(schedule.section)}`
                });
              });
            }
          });
